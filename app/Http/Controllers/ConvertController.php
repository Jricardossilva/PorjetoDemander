...
class ConvertController extends Controller
{
    public function convert(Request $request) {
        ...
        } elseif(preg_match("/^M{0,3}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})$/", strtoupper($request -> num))){
            $valor = strtoupper($request -> num);
    ...
    public function convert_romanos($numero) {
        $resultado = '';
        foreach ($this -> base_romanos as $romano => $valor) {
            while ($numero >= $valor) {
                $resultado .= $romano;
                $numero -= $valor;
            }
    ...
    public function convert_real($numero) {
        ...
        $x = 0;
        while ($x < strlen($romano)) {
            $caracter_romano = $romano[$x];
            $proximo = ($x + 1 < strlen($romano)) ? $romano[$x + 1] : null;
